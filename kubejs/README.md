...
| `startup_scripts/consortium_products.js`, `server_scripts/consortium_products.js`, `assets/consortium/textures/item/` | GENERATED by `tools/products/build-products.mjs` (docs/BOARD_ORDERS.md): the 14 Board Orders products and 17 intermediates as plain items with generated textures (startup, both sides), and their roadmap recipes (server): Create mixing, sequenced assembly, mechanical crafting, filling; Farmer's Delight cooking; smoking; Productive Metalworks casting; IE blueprint, metal press, arc furnace, bottling; Crafts and Additions charging; Powah energizing; Mekanism infusing, combining, injecting, nucleosynthesizing; AE2 inscribing; the NASA workbench. Never edit by hand |
